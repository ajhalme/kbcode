...
/**
 **    Advanced Kilobot magic, a form renderer
 **    Relies heavily on "libkbutil"
 **
...
// follow: INIT - ELECT - SCATTER - GET/ORBIT/SPIRAL/FINISH - HOLD
enum FSM {
	// leader election
	INIT,
	ELECT,
...
void state_finish(void) {	
    ...
    if (((mark == 1) && heardR2bound) || (heardR1bound && (id == 1))) {
        ...
        top = id;
        if (id == N - 1) {
            message_out(id, top, MODE_DONE);
            LED_GREEN;
        } else {
            message_out(id, top, MODE_GO);
            LED_BLUE;
        }
        enable_tx = 1;
        state = HOLD;
...
void state_hold(void) {
	...
	if (HEARD) {
        ...
        if (mtop > top && (mmode == MODE_GO || mmode == MODE_DONE)) { 
            top = mtop;
    ...
    kprints("stateIDt");
  	kprinti(state); kprinti(id); kprinti(top);
	NEWLINE;
}
